...
#' from the resource's `/metadata` endpoint via direct HTTP calls.
...
#' Requires `geosphere_get_datasets` function to be available.
#'
...
#' @param timeout_seconds Numeric. Timeout in seconds for HTTP requests. Defaults to 2.
#'
...
#' }
#' }
geosphere_explore_resource = function(
  resource_id,
  ...
  cli::cli_alert_info(
    "Looking up all combinations for resource ID: {.val {resource_id}}"
  )

  # Ensure geosphere_get_datasets exists <--- CORRECTED
  if (!exists("geosphere_get_datasets", mode = "function")) {
    # <--- CORRECTED
    stop(
      "Dependency function 'geosphere_get_datasets' not found.",
      call. = FALSE
    ) # <--- CORRECTED (message)
  }
  ...
  if (!"resource_id" %in% names(all_datasets)) {
    stop(
      "Could not find 'resource_id' column in the dataset list retrieved by geosphere_get_datasets. Ensure 'add_resource_id = TRUE'.",
      call. = FALSE
    )
  }
  ...
  if (nrow(matching_datasets) == 0) {
    cli::cli_alert_danger(glue::glue(
      "Resource ID '{resource_id}' not found in the available datasets list."
    ))
    return(NULL)
  }
  ...
  cols_to_check = intersect(
    c("type", "mode", "response_formats"),
    names(matching_datasets)
  )
  if (!all(c("type", "mode") %in% cols_to_check)) {
    stop(
      "Internal error: Could not find 'type' and 'mode' columns in the dataset list.",
      call. = FALSE
    )
  }
  ...
  if (
    "response_formats" %in%
      cols_to_check &&
      is.list(matching_datasets$response_formats)
  ) {
    ...
    matching_datasets = dplyr::mutate(
      matching_datasets,
      response_formats_str = as.character(.data$response_formats)
    )
    cols_for_distinct = c("type", "mode", "response_formats_str")
  } else {
    # Handle case where response_formats column might be missing entirely
    cli::cli_alert_warning(
      "Column 'response_formats' not found in dataset list for resource ID {.val {resource_id}}. Format part of combination name will be 'unknown'."
    )
    matching_datasets = dplyr::mutate(
      matching_datasets,
      response_formats_str = "unknown"
    )
    cols_for_distinct = c("type", "mode", "response_formats_str")
  }
  ...
  unique_combinations = dplyr::distinct(
    matching_datasets,
    dplyr::pick(dplyr::all_of(cols_for_distinct))
  )

  n_combinations = nrow(unique_combinations)
  cli::cli_alert_info(
    "Found {n_combinations} unique type/mode/format combination{?s} for resource ID {.val {resource_id}}."
  )
  if (n_combinations > 1) {
    cli::cli_alert_warning(
      "Multiple combinations found. Details for each will be provided."
    )
  } else if (n_combinations == 0) {
    # This case should theoretically not happen if matching_datasets > 0 rows, but good to check
    cli::cli_alert_danger(
      "Internal error: No unique combinations found despite matching datasets existing."
    )
    return(NULL)
  }
  ...
  for (i in 1:n_combinations) {
    ...
    cli::cli_h1(
      "Processing Combination: {.val {list_name}} ({i}/{n_combinations})"
    )

    # --- 4a. Fetch and Interpret Metadata ---
    cli::cli_alert_info("Fetching descriptive metadata...")

    # Construct metadata URL directly
    metadata_path = paste(
      version,
      type,
      mode,
    ...
    current_metadata = tryCatch(
      {
        ...
        if (
          ...
          warning(
            "API did not return JSON content as expected for metadata.",
            call. = FALSE
          )
        }
      ...
      error = function(e) {
        cli::cli_alert_warning(
          "Failed to fetch metadata for combination {.val {list_name}}."
        )
    ...
    output_element = list(
      combination = list(
        type = type,
        mode = mode,
        formats = strsplit(formats_str, "_")[[1]]
      ),
    ...
    if (is.null(current_metadata)) {
      cli::cli_alert_warning(
        "Could not fetch or parse metadata for combination {.val {list_name}}."
      )
      ...
      if (!is.null(current_metadata$parameters)) {
        if (
          is.data.frame(current_metadata$parameters) &&
            nrow(current_metadata$parameters) > 0
        ) {
          output_element$available_parameters = tibble::as_tibble(
            current_metadata$parameters
          )
        } else if (
          is.list(current_metadata$parameters) &&
            length(current_metadata$parameters) > 0 &&
            !is.data.frame(current_metadata$parameters)
        ) {
          # Attempt to handle list of lists case
          param_df = tryCatch(
            dplyr::bind_rows(current_metadata$parameters),
            error = function(e) NULL
          )
          if (!is.null(param_df) && nrow(param_df) > 0) {
            output_element$available_parameters = tibble::as_tibble(param_df)
          }
        }
      }
      # Extract Time Coverage
      time_info = list()
      if (!is.null(current_metadata$start_time)) {
        time_info$start_time = current_metadata$start_time
      }
      if (!is.null(current_metadata$end_time)) {
        time_info$end_time = current_metadata$end_time
      }
      if (!is.null(current_metadata$timerange)) {
        time_info$timerange = current_metadata$timerange
      }
      if (length(time_info) > 0) {
        output_element$time_coverage = time_info
      }
      # Extract Spatial Info
      spatial = list()
      if (!is.null(current_metadata$crs)) {
        spatial$crs = current_metadata$crs
      }
      if (!is.null(current_metadata$bbox)) {
        spatial$bbox = current_metadata$bbox
      }
      if (!is.null(current_metadata$spatial_resolution_m)) {
        spatial$resolution_m = current_metadata$spatial_resolution_m
      }
      if (!is.null(current_metadata$grid_bounds)) {
        spatial$grid_bounds = current_metadata$grid_bounds
      }
      if (length(spatial) > 0) output_element$spatial_info = spatial
    }

    # --- 4b. Probe Data Endpoint for Required Parameters ---
    cli::cli_alert_info(
      "Probing data endpoint to determine required parameters..."
    )
    ...
    probe_response = tryCatch(
      ...
      error = function(e) {
        cli::cli_alert_danger(
          "HTTP request failed when probing data URL {.url {data_url}}"
        )
        cli::cli_alert_danger("Error: {e$message}")
      ...
      if (status >= 400 && status < 500) {
        ...
        if (nzchar(error_text)) {
          error_content = tryCatch(
            ...
            error = function(e) {
              cli::cli_alert_warning(
                "Could not parse error response from data endpoint as JSON."
              )
        ...
        if (
          !is.null(error_content) &&
            is.list(error_content) &&
            !is.null(error_content$detail)
        ) {
          ...
          if (is.data.frame(details_list) && "loc" %in% names(details_list)) {
            if (is.list(details_list$loc)) {
              # Check if loc column is itself a list
              parsed_params = sapply(details_list$loc, function(loc_item) {
                if (
                  is.character(loc_item) &&
                    length(loc_item) >= 2 &&
                    loc_item[1] == "query"
                ) {
                  return(loc_item[2])
              ...
              req_params_list = c(
                req_params_list,
                parsed_params[!is.na(parsed_params)]
              )
            }
          } else if (is.list(details_list)) {
            # Handle if detail is a list of lists
            parsed_params = sapply(details_list, function(item) {
              if (
                is.list(item) &&
                  !is.null(item$loc) &&
                  is.character(item$loc) &&
                  length(item$loc) >= 2 &&
                  item$loc[1] == "query"
              ) {
                return(item$loc[2])
            ...
            if (
              ...
              found_in_string = stringr::str_extract_all(
                details_list,
              ...
              if (length(found_in_string) > 0) {
                ...
                cli::cli_alert_info(
                  "Attempted to parse required params from error string: {.val {paste(found_in_string, collapse=', ')}}"
                )
              }
            }
          }
        ...
        if (is.null(required_params_found)) {
          cli::cli_alert_warning(
            "Received HTTP {status} error, but could not extract required parameters from the response body."
          )
          ...
          cli::cli_alert_info(
            "Received HTTP {status} error, but the response indicated no specific missing query parameters (or format was unparseable in detail)."
          )
        ...
        cli::cli_alert_warning(
          "Probing data endpoint did not return expected 4xx error (Status: {status}). Cannot reliably determine required parameters this way."
        )
    ...
    if (print_summary) {
      ...
      if (!is.null(output_element$metadata$title)) {
        cli::cli_text("Title: {.val {output_element$metadata$title}}")
      }
      cli::cli_text("Data Query URL (Base): {.url {data_url}}")
      if (!is.null(output_element$metadata)) {
        cli::cli_text("Metadata URL: {.url {paste0(data_url,'/metadata')}}")
      } # Assumes metadata URL structure

      cli::cli_h2("Available Parameters (from Metadata)")
      if (
        !is.null(output_element$available_parameters) &&
          nrow(output_element$available_parameters) > 0
      ) {
        cols_to_print = intersect(
          c("name", "long_name", "unit", "desc"),
          names(output_element$available_parameters)
        )
        print_df = if (length(cols_to_print) > 0) {
          output_element$available_parameters[, cols_to_print, drop = FALSE]
        } else {
          output_element$available_parameters
        }
        # Ensure tibble printing is used if available
        print(print_df, n = 6) # Use tibble's print method which handles head implicitly
        if (nrow(print_df) > 6) {
          cli::cli_text("({nrow(print_df) - 6} more parameters available...)")
        }
      } else {
        cli::cli_text(
          "No specific parameter list found in metadata or list was empty."
        )
      }

      cli::cli_h2("Time Coverage (from Metadata)")
      if (
        !is.null(output_element$time_coverage) &&
          length(output_element$time_coverage) > 0
      ) {
        cli::cli_ul()
        lapply(names(output_element$time_coverage), function(n) {
          cli::cli_li(
            "{tools::toTitleCase(gsub('_',' ',n))}: {.val {paste(output_element$time_coverage[[n]], collapse=', ')}}"
          )
        })
        cli::cli_end()
      } else {
        cli::cli_text("No specific time coverage info found.")
      }

      cli::cli_h2("Spatial Information (from Metadata)")
      if (
        !is.null(output_element$spatial_info) &&
          length(output_element$spatial_info) > 0
      ) {
        cli::cli_ul()
        lapply(names(output_element$spatial_info), function(n) {
          value = output_element$spatial_info[[n]]
          formatted_value = if (is.numeric(value)) {
            paste(round(value, 4), collapse = ', ')
          } else {
            paste(value, collapse = ', ')
          }
          cli::cli_li(
            "{tools::toTitleCase(gsub('_',' ',n))}: {.val {formatted_value}}"
          )
        })
        cli::cli_end()
      } else {
        cli::cli_text("No specific spatial info found.")
      }

      cli::cli_h2(
        "Required Query Parameters for {.fn geosphere_get_data} (determined by probing)"
      )
      if (
        !is.null(output_element$required_query_params) &&
          length(output_element$required_query_params) > 0
      ) {
        cli::cli_alert_info(
          "Parameters needed in the `query` argument for {.fn geosphere_get_data}:"
        )
        cli::cli_ul()
        for (p in output_element$required_query_params) {
          cli::cli_li("{.code {p}}")
        }
        cli::cli_end()
      } else if (is.null(output_element$required_query_params)) {
        cli::cli_alert_warning(
          "Could not determine required parameters (probe failed, response unparseable, or other non-4xx status)."
        )
      } else {
        # length is 0
        cli::cli_alert_info(
          "Probe suggests no specific query parameters are required (or endpoint works differently, or error format was unrecognizable)."
        )
      }
