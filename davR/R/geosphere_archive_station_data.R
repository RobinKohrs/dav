...
#' @param download_all_params Logical. If `TRUE`, downloads data for all available parameters
#'   for the resource instead of using the `params` argument. Requires user confirmation in
#'   interactive sessions. Defaults to `FALSE`.
#' @param interactive Logical. If `TRUE` (default), prompts for user confirmation when
#'   `download_all_params = TRUE`. If `FALSE`, proceeds automatically with all parameters.
#'
...
#'   )
#'
#'   # Download all available parameters (with confirmation prompt)
#'   geosphere_archive_station_data(
#'     resource_id = "klima-v2-1d",
#'     download_all_params = TRUE,
#'     start_date = "2024-01-01",
#'     end_date = "2024-01-31"
#'   )
#'
...
#'   )
#' }
#' @importFrom cli cli_h1 cli_h2 cli_alert_info cli_alert_danger cli_alert_success cli_progress_bar cli_progress_update cli_progress_done cli_text cli_ul cli_li cli_end cli_alert_warning
...
geosphere_archive_station_data <- function(
  ...
  for (attempt in 1:5) {
    ...
    all_stations_df <- geosphere_get_stations(
      metadata_url = metadata_url,
      return_format = "dataframe"
    )

    if (
      !is.null(all_stations_df) &&
        is.data.frame(all_stations_df) &&
        nrow(all_stations_df) > 0
    ) {
  ...
  if (
    ...
    cli::cli_alert_danger(
      "Could not retrieve station metadata or metadata is empty. Aborting."
    )
    return(invisible(NULL))
  }
  if (!all(c("id", "valid_from") %in% names(all_stations_df))) {
    cli::cli_alert_danger(
      "Metadata missing 'id' or 'valid_from' columns. Aborting."
    )
    return(invisible(NULL))
  }

  target_stations_df <- if (is.null(station_ids)) {
    all_stations_df
  } else {
    all_stations_df[all_stations_df$id %in% station_ids, ]
  }
  ...
  cli::cli_alert_success(paste0(
    "Found ",
    nrow(target_stations_df),
    " target stations."
  ))

  # --- 2. HANDLE ALL PARAMETERS OPTION ---
  if (download_all_params) {
    cli::cli_alert_info(
      "Fetching all available parameters for resource: {.val {resource_id}}"
    )

    # Get all available parameters using the new function
    all_params_df <- tryCatch(
      {
      ...
      error = function(e) {
        cli::cli_alert_danger(
          "Failed to fetch available parameters: {e$message}"
        )
        return(NULL)
      }
    )

    if (!is.null(all_params_df) && nrow(all_params_df) > 0) {
      # Extract parameter names
      all_param_names <- all_params_df$name[!is.na(all_params_df$name)]

      # Remove flag parameters to avoid duplicates and focus on actual measurements
      all_param_names <- all_param_names[!grepl("_flag$", all_param_names)]

      cli::cli_alert_info(
        "Found {length(all_param_names)} available parameters (excluding quality flags)"
      )

      # Confirmation prompt
      if (interactive && base::interactive()) {
        cli::cli_text("")
        cli::cli_h2("⚠️  Download Confirmation Required")
        cli::cli_text(
          "You are about to download data for {.strong {length(all_param_names)}} parameters:"
        )
        cli::cli_text("")

        # Show first few parameters as preview
        preview_params <- head(all_param_names, 10)
        cli::cli_ul()
        for (param in preview_params) {
          param_info <- all_params_df[all_params_df$name == param, ]
          if (nrow(param_info) > 0 && !is.na(param_info$long_name[1])) {
            cli::cli_li("{.code {param}} - {param_info$long_name[1]}")
          } else {
            cli::cli_li("{.code {param}}")
          }
        }
        cli::cli_end()

        if (length(all_param_names) > 10) {
          cli::cli_text(
            "... and {length(all_param_names) - 10} more parameters"
          )
        }
        cli::cli_text("")

        # Calculate rough estimates (exact values calculated later)
        rough_time_periods <- if (!is.null(start_date) && !is.null(end_date)) {
          as.numeric(as.Date(end_date) - as.Date(start_date)) / 30 # Rough monthly estimate
        } else {
          12 # Default to ~1 year of monthly data
        }
        rough_station_batches <- ceiling(nrow(target_stations_df) / 10) # Assume ~10 stations per batch
        estimated_requests <- max(1, rough_time_periods * rough_station_batches)
        estimated_time_hours <- (estimated_requests * api_delay_secs) / 3600

        cli::cli_text("Estimated download (rough):")
        cli::cli_ul()
        cli::cli_li("Parameters: {.strong {length(all_param_names)}}")
        cli::cli_li("Stations: {.strong {nrow(target_stations_df)}}")
        cli::cli_li("API requests: {.strong ~{round(estimated_requests)}}")
        cli::cli_li(
          "Estimated time: {.strong ~{round(estimated_time_hours, 2)} hours}"
        )
        cli::cli_end()
        cli::cli_text("")
        ...
        response <- utils::menu(
          choices = c(
            "Yes, download all parameters",
            "No, use default parameters",
            "Cancel"
          ),
          title = "Do you want to proceed with downloading ALL parameters?"
        )

        if (response == 1) {
          params <- all_param_names
          cli::cli_alert_success("Proceeding with {length(params)} parameters")
        } else if (response == 2) {
          cli::cli_alert_info(
            "Using default parameters: {.val {paste(params, collapse = ', ')}}"
          )
        } else {
          cli::cli_alert_info("Download cancelled by user")
          return(invisible(NULL))
        }
      } else {
        ...
        cli::cli_alert_warning(
          "Non-interactive mode: Automatically using all {length(params)} parameters"
        )
      }
    } else {
      cli::cli_alert_warning(
        "Could not retrieve parameter list. Using default parameters."
      )
    }
  }

  # --- 3. OPTIMIZE BATCH SIZES BASED ON RATE LIMITS ---
  ...
  if (is.null(station_batch_size)) {
    ...
    station_batch_size <- max(
      1,
      floor(max_values_per_request / (length(params) * time_steps_per_batch))
    )

    cli::cli_alert_info(paste0(
      "Calculated optimal station batch size: ",
      station_batch_size,
      " (",
      length(params),
      " params × ~",
      round(time_steps_per_batch),
      " time steps)"
    ))
  }
  ...
  if (api_delay_secs < 0.2) {
    cli::cli_alert_warning(
      "API delay increased to 0.2 seconds to respect 5 requests/second limit"
    )
    api_delay_secs <- 0.2
  }
  ...
  station_batches <- split(
    target_stations_df$id,
    ceiling(seq_along(target_stations_df$id) / station_batch_size)
  )
  ...
  cli::cli_alert_info(paste0(
    "Estimated time: ~",
    round(total_batches * api_delay_secs / 60, 1),
    " minutes"
  ))
  ...
  for (time_batch in time_batches) {
    for (station_batch in station_batches) {
      ...
      station_hash <- substr(
        digest::digest(
          paste(sort(station_batch), collapse = "_"),
          algo = "md5"
        ),
        1,
        8
    ...
    for (attempt in 1:3) {
      ...
      if (is.list(result) && !is.null(result$result)) {
        ...
        if (!is.null(result$rate_limit_info)) {
          remaining_second <- as.numeric(
            result$rate_limit_info$remaining_second
          ) %||%
            "unknown"
          remaining_hour <- as.numeric(
            result$rate_limit_info$remaining_hour
          ) %||%
            "unknown"
          cli::cli_alert_info(glue::glue(
            "Rate limit: {remaining_second} req/sec, {remaining_hour} req/hour remaining"
          ))
        }
        ...
        split_batch_file(
          result$result,
          batch$resource_archive_path,
          batch$resource_id,
          batch$time_batch_size
        )
        break
      } else {
        ...
        if (attempt < 3) {
          ...
          if (is.list(result) && !is.null(result$rate_limit_info)) {
            reset_seconds <- as.numeric(
              result$rate_limit_info$reset_seconds
            ) %||%
              3600
            wait_time <- min(reset_seconds + 10, 300) # Wait for reset + 10s, max 5 minutes
            cli::cli_alert_warning(paste0(
              "Rate limit exceeded. Waiting ",
              round(wait_time / 60, 1),
              " minutes for reset..."
            ))
...
create_time_batches <- function(
  ...
  if (is.null(start_date)) {
    ...
    if (length(valid_dates) > 0) {
      ...
      cli::cli_alert_warning(
        "No valid 'valid_from' dates found in station metadata. Using 1 year ago as start date."
      )
    }
  } else {
  ...
  if (earliest_date > end_date) {
    ...
    cli::cli_alert_warning(
      "Start date is after end date. Using 30 days ago as start date."
    )
  }

  cli::cli_alert_info(paste0(
    "Creating date sequence from ",
    earliest_date,
    " to ",
    end_date,
    " by ",
  ...
  tryCatch(
    {
      ...
      if (is.null(station_col)) {
        cli::cli_alert_warning(
          "Could not identify station column in batch file"
        )
        return()
      }
      ...
      for (station_id in stations) {
        ...
        station_filename <- paste0(
          ...
          station_id,
          "_",
          format(as.Date(station_data$time[1]), "%Y-%m-%d"),
          ".csv"
        )
        station_path <- file.path(station_dir, station_filename)
        ...
        if (
          !file.exists(station_path) ||
            file.size(station_path) < nrow(station_data) * 100
        ) {
